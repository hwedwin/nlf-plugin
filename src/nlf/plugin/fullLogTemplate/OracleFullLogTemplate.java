...
import nc.liat6.frame.db.custom.oracle.IOracle;
 ...
 *
 */
public class OracleFullLogTemplate extends CommonFullLogTemplate implements IOracle{
  ...
  public PageData query(String sql,int pageNumber,int pageSize,Object param){
    ...
    String nsql = "SELECT * FROM (SELECT NLFTABLE_.*,ROWNUM RN_ FROM ("+sql+") NLFTABLE_ WHERE ROWNUM <= "+(d.getPageNumber()*d.getPageSize())+") WHERE RN_ > "+((d.getPageNumber()-1)*d.getPageSize());
    List<Object> pl = processParams(param);
    String logSql = nsql;
    for(Object p:pl){
      logSql = logSql.replaceFirst("\\?","'"+p+"'");
  ...
  public List<Object[]> top(String sql,Object param,int n){
    flush();
    String nsql = "SELECT * FROM ("+sql+") WHERE ROWNUM <= "+n;
    List<Object> pl = processParams(param);
    String logSql = nsql;
    for(Object p:pl){
      logSql = logSql.replaceFirst("\\?","'"+p+"'");
